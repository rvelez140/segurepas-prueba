...
const QRModal = forwardRef<HTMLDivElement, QRModalProps>(({ isOpen, visit, onClose }, ref) => {
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      const file = new File([blob], `Autorización-${actualVisit.visit.name}.png`, {
        type: 'image/png',
      });
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        ...
        const message =
          ...
          `Fecha: ${actualVisit.authorization.date.toLocaleDateString('es-ES')}\n` +
          (actualVisit.authorization.exp
            ? `Vence: ${actualVisit.authorization.exp.toLocaleDateString('es-ES')}\n`
            : '');
      ...
      <div className={styles.qrModal} ref={ref}>
        ...
        {qr ? (
          <div className={styles.qrContainer}>
            ...
            <p>
              {new Date(actualVisit.authorization.date).toLocaleDateString('es-ES', {
                year: 'numeric',
                month: 'long',
                day: 'numeric',
              })}
            ...
            {actualVisit.authorization.exp && (
              <span>
                Vence:{' '}
                {new Date(actualVisit.authorization.exp).toLocaleDateString('es-ES', {
                  year: 'numeric',
                  month: 'long',
                  day: 'numeric',
                })}

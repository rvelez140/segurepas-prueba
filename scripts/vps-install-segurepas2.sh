...
#######################################
# Script de Instalación en VPS - Segurepas2
# Instala Git, Docker y configura el proyecto en /opt/segurepas2
...
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}  SecurePass2 - Instalación en VPS${NC}"
echo -e "${GREEN}========================================${NC}"
echo ""
...
if command -v ufw &> /dev/null; then
    ...
    ufw allow 8472/tcp
    ufw allow 8473/tcp
    show_success "Firewall configurado (puertos 22, 80, 443, 8472, 8473 abiertos)"
else
...
if [ "$ssl_answer" = "y" ] || [ "$ssl_answer" = "Y" ]; then
    ...
    echo -e "  ${BLUE}certbot certonly --standalone -d tudominio2.com -d api.tudominio2.com${NC}"
else
...
cat > /etc/logrotate.d/segurepas2 <<EOF
...
echo -e "   • FRONTEND_URL y REACT_APP_API (tus dominios)"
echo -e "   • NGINX_HTTP_PORT=8472 (puerto diferente para evitar conflictos)"
echo -e "   • NGINX_HTTPS_PORT=8473 (puerto diferente para evitar conflictos)"
echo ""
echo -e "${BLUE}3. Si usas SSL con Certbot:${NC}"
echo -e "   sudo certbot certonly --standalone -d tudominio2.com -d api.tudominio2.com"
echo -e "   Luego copia los certificados:"
echo -e "   sudo mkdir -p $DEPLOY_PATH/nginx/ssl"
echo -e "   sudo cp /etc/letsencrypt/live/tudominio2.com/fullchain.pem $DEPLOY_PATH/nginx/ssl/"
echo -e "   sudo cp /etc/letsencrypt/live/tudominio2.com/privkey.pem $DEPLOY_PATH/nginx/ssl/"
...
echo -e "   sudo docker build -t segurepas2-api:latest ./apps/api"
echo -e "   sudo docker build -t segurepas2-web:latest ./apps/web"
echo ""
...
echo -e "${YELLOW}NOTA: Esta instalación usa puertos diferentes (8472, 8473) para evitar conflictos${NC}"
echo ""
